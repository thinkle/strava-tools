<script type="typescript">
  ...
  $: if (autoUpdate) $startDate = useStartDate && new Date(start) || 0; // react to string changes
  $: if (autoUpdate) $endDate = useEndDate && new Date(end) || 0; // react to string changes
...
<div class="wrap">
  ...
  {#if useStartDate && useEndDate && start > end}
    <div transition:fade class="warning">Oops! Your start date is after your end date!</div>
  {/if}
</div>
