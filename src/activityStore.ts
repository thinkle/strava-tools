...
export const startDate = writable<Date|0>(new Date(
  now.getFullYear(),now.getMonth(),1,0)
); // first of the month
const fetching = writable(false);

function modifyFetcherForRedundancy(currentParams : ActivityFetcherParams,$fetchersByDate : Map<string,ActivityFetcherParams>) : ActivityFetcherParams {
  let {oldest,newest} = getBaseFetchInfo();
  let updates = {}; 
  if (newest) { // If we have base fetches already, let's assume they were in order from present to past...
  ...
  if (!updates.complete) {
    ...
    $fetchersByDate.forEach(
      (fetcherParams,stampString)=>{
...
async function fetchActivities (fetcher : ActivityFetcherParams) : Promise<Set<number>> {
  if (!fetcher.complete) {
    let newActivities = await getActivities(
      fetcher.page, fetcher.per_page, fetcher.before, fetcher.after
    );
    //console.log('fetchActivities=>',logF(fetcher),fetcher,newActivities)
    activitiesById.update(
      ($activitiesById)=>{
...
async function prepopulateActivities (fetcher : ActivityFetcherParams, $activitiesById : Map<number,Activity>) {
  ...
  ([$fetchersByDate,$activitiesById,$startDate,$endDate])=>{ 
    ...
    let currentFetcher = {
      ...
      activities : ()=>{
        ...
        let result = Array.from(fetcher.activities.keys(),(id)=>$activitiesById.get(id))
        return result;
      },
      fetchMore : async ()=>{
        let fetcher = currentFetcher.getFetcher();
